...
const topic: Topic = {
  ...
  sections: [
    ...
    },
    {
      ...
      title: 'One Word, Six Contracts: Coffee · Cotton · Sugar · Wheat · Brent · WTI',
      body: `"Futures contract" is one legal form wrapped around very different physical realities. The matrix below puts six real contracts **side by side, one column each**, with the spec rows aligned so you can read straight across: the **next five listed delivery months** (who trades in contango, who in backwardation; Brent and WTI list *every* month while Sugar lists four a year and Wheat follows the crop calendar), then **origin spec**, **quality spec**, **delivery points** and **Incoterm**.\n\nThe contrasts to find:\n\n- **Origin:** from *US-grown only* (Cotton) to *~28 origins* (Sugar) to *EU milling wheat* (the "blé MATIF" every French farmer hedges on) to *no origin at all* (Brent)\n- **Delivery:** warehouse warrants (Coffee, Cotton), an in-silo warrant at Rouen/Dunkirk (Wheat), *FOB the buyer's vessel at the origin port* (Sugar — the buyer charters the ship!), one inland pipeline hub (WTI at Cushing), and pure cash settlement (Brent)\n- **The crude pair:** Brent and WTI are the same size and the same quote — yet one settles against a seaborne index and the other physically delivers at a single inland hub. That difference is why they can diverge, and it is what produced WTI's famous **negative price in April 2020**\n- **Currency:** five contracts in US units — and one in **€/t**\n\nBefore trading any contract, read its rulebook: the word "futures" tells you almost nothing about what you would actually receive.`,
