...
const topic: Topic = {
  ...
  sections: [
    ...
    {
      id: 'no-exchange',
      title: 'A World Without an Exchange: Olive Oil & Pepper',
      body: `Is the exchange really necessary? Run the thought experiment below: slide the number of participants and toggle between a **bilateral world** — where every producer must find and price against every buyer — and a world with **one exchange**. The negotiation burden goes from quadratic to linear; the price goes from private to public.\n\nThen look at the markets that **never got a liquid futures contract**: olive oil, pepper, rice. When Spanish drought sent extra-virgin olive oil from ~\u20ac2,000/t to nearly \u20ac9,000/t and back down, every importer and bottler simply **ate the move** — no hedge to lay it off, no transparent reference to negotiate against, no buyer of last resort for stuck stock, and suppliers walking away from old contracts. The volatility is not worse than coffee's; the difference is that **coffee traders can do something about it**.`,
